    ...
    for _ in range(num_epochs):
        for batch in train_dataloader:
            ...
            if step % log_interval == 0:
                print(f"step {step}  avg_loss {running_loss / log_interval:.4f}")
                running_loss = 0.0
...
def create_augmented_dataloader(args, dataset):
    ...
    n = len(train_split)
    rng = random.Random(0)
    sample_idx = rng.sample(range(n), min(5000, n))
    aug_texts = []
    aug_labels = []
    for j in sample_idx:
        ex = train_split[j]
        aug_ex = custom_transform({"text": ex["text"], "label": ex["label"]}, idx=j)
        aug_texts.append(aug_ex["text"])
        aug_labels.append(aug_ex["label"])
    aug_ds = Dataset.from_dict(
        {"text": aug_texts, "label": aug_labels},
        features=train_split.features,
    )
    combined = concatenate_datasets([train_split, aug_ds])
    tokenized_aug = combined.map(tokenize_function, batched=True, load_from_cache_file=False)
...
def create_transformed_dataloader(args, dataset, debug_transformation):
    ...
    if debug_transformation:
        ...
        small_transformed_dataset = small_dataset.map(
            lambda ex, i: custom_transform(ex, i), with_indices=True, load_from_cache_file=False
        )
        for k in range(5):
